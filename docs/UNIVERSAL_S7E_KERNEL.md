...
- O runtime tune agora tenta manter o teto do big core em `3016000` sempre (`LOCK_BIG_MAX_TO_TARGET=1`), sem fallback automático para bins menores.
- Se você quiser comportamento dinâmico estilo **2.8 ↔ 3.016 GHz** (sobe em burst e reduz quando esquenta), rode o guard adaptativo em background:
  ...
  ```
  - Ajustes opcionais: `HIGH_KHZ`, `LOW_KHZ`, `TEMP_HIGH_MILLIC`, `TEMP_LOW_MILLIC`, `INTERVAL_MS`, `LOCK_MAX_ALWAYS`.
  - Padrão estrito: `HIGH_KHZ=3016000` e `LOW_KHZ=3016000` (teto fixo de 3.016 GHz no big core).
  - Exemplo estrito: `HIGH_KHZ=3016000 LOW_KHZ=3016000 su -c sh /path/para/kernel/scripts/oc_adaptive_guard.sh &`
  - Por padrão `LOCK_MAX_ALWAYS=1`: o teto do big core fica travado no valor de `HIGH_KHZ` (use `LOCK_MAX_ALWAYS=0` se quiser reduzir para `LOW_KHZ` quando aquecer).
