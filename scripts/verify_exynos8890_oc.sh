...
if [ -z "${TARGET}" ]; then
	TARGET="$(echo "${AVAIL_FREQS}" | head -n1)"
	echo "WARN: 3016 MHz not exposed. Testing top available bin: ${TARGET} KHz"
fi

echo "Target frequency: ${TARGET} KHz"
echo "Duration: ${DURATION_SEC}s, sample period: ${SAMPLE_MS}ms"
...
echo "Observed peak: ${peak} KHz"

tolerance=16000
if [ "${peak}" -ge $((TARGET - tolerance)) ]; then
	echo "PASS: reached target bin (within ${tolerance} KHz tolerance)."
	exit 0
fi

echo "FAIL: did not reach requested top bin."
echo "Hint: check thermal throttling, voltage limits, and silicon bin quality."
exit 1
